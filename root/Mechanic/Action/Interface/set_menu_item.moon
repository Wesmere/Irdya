...
wsl_action
    ...
    scheme:
        ...
        needs_select:
            description: [[if yes (default no), then the latest select event (see EventWML) that triggered before this menu item was chosen will be transmitted over the network before this menu item action will be. This only has any effect in networked multiplayer, and is intended to allow more elaborate menu item behaviour there without causing out of sync errors. If you don't know what this means, just leave it false.]]
        ...
        show_if:
            description: [[If present, the menu item will only be available if the conditional statement (see ConditionalActionsWML) within evaluates to true. When this is evaluated, the WML variables $x1 and $y1 will point to the location on which the context menu was invoked, so it's possible to for example only enable the option on empty hexes or on a particular unit.]]

        filter_location:
            description: [[contains a location filter similar to the one found inside Single Unit Filters (see FilterWML). The menu item will only be available on matching locations.]]

        default_hotkey:
            description: [[contains a hotkey WML to specify what hotkey to assign to this, if the user has no hotkey assigned to this yet. (Unlike the rest of a menu item definition, modifying this tag has no effect on the game; it is only effective when initially defining a menu item.) Hotkey WML matches the format in the preferences file and contains the following keys:
key: a string that contains the key to assign to this.
alt, shift, cmd(apple only), ctrl: boolean values.]]

        command:
            description: [[contains the WML actions to be executed when the menu item is selected. Again, the WML variables $x1 and $y1 will point to the location on which the context menu was invoked on.]]

        delayed_variable_substitution:
            description: [[(boolean yes|no, default: yes): If no, forces a variable substitution run onto the wml included in this [command] block. Use this, if you want variables which are to substitute to get the values they have at execution time of the event where this set_menu_item appears. Other than that, they get the values they have at invocation time of the menu item.]]
